 ...
 */
public class BidProjOnMngForPchsChecker {


    public static String checkArgsForSaveOrSub(IoBidProjOnDto dto) {
        ...
        List<IoBidProjOnMatInfDto> matList = dto.getMatList();
        if (matList == null || matList.isEmpty()) {
            sb.append("物料信息不得为空;");
        } else {
            int i = 1;
            for (IoBidProjOnMatInfDto matInfDto : matList) {
    ...
    public static String checkArgsForAdtBidInf(IoBidProjOnBidAdtDto dto) {
        StringBuilder sb = new StringBuilder("");
    ...
    public static String checkArgsForChsExpts(IoBidProjOnChsExptsDto dto) {
        StringBuilder sb = new StringBuilder("");
        ...
        String exptIds = dto.getExptIds();
        if (StringUtils.isEmpty(exptIds)) {
            sb.append("专家IDs不得为空;");
        }
    ...
    public static String checkArgsForOpenProj(IoBidProjOnOpenDto dto) {
    ...
    public static String checkArgsForStartQot2(IoStartQot2Dto dto) {
        ...
        String qot2EndTimStr = dto.getQot2EndTimStr();
        if (StringUtils.isEmpty(qot2EndTimStr)) {
            sb.append("二次报价截止时间不得为空;");
        } else {
            try {
                Timestamp qot2EndTim = DateUtils.getTimeStamp(qot2EndTimStr);
                dto.setQot2EndTim(qot2EndTim);
            } catch (Exception e) {
                sb.append("二次报价截止时间格式不正确;");
            }
        }

        String qot2CntTyp = dto.getQot2CntTyp();
        if (StringUtils.isEmpty(qot2CntTyp)) {
            sb.append("二次竞价报价次数类型不得为空;");
        }

        String splrIds = dto.getSplrIds();
        if (StringUtils.isEmpty(splrIds)) {
            sb.append("邀请二次竞标供应商IDs不得为空;");
        }

        return sb.toString();
    }

    public static String checkArgsForGetGrdInf(IoShowGrdInfDto dto) {
    ...
    public static String checkArgsForAppAwd(IoAppAwdDto dto) {
        ...
        List<IoAppAwdDtlDto> appAwdDtls = dto.getAppAwdDtls();

        if (appAwdDtls == null || appAwdDtls.isEmpty()) {
            sb.append("决标明细不得为空;");
        } else {
            int i = 0;
            for (IoAppAwdDtlDto dtl : appAwdDtls) {
